...
/*
fn fn_param_typs<'tcx>(typ: rustc_middle::ty::Ty<'tcx>) { //-> (Vec<Typ>, Typ) {
    println!("typ: {:#?}", typ);
    match typ.kind() {
        TyKind::FnDef(_def, substs) => {
            println!("substs: {:#?}", substs);
            let typs = substs.types();
            for typ in typs {
                println!("   typ: {:#?}", typ);
            }
        ...
        _ => panic!("closure_param_types expected FnDef type"),
    }
}
*/
